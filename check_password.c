...
#if defined(DEBUG)
						syslog(LOG_NOTICE,
							   "check_password: Word = %s, value = %s", word,
							   value);
#endif
#if defined(LDEBUG)
...
int check_password(char* pPasswd, struct berval* errmsg, Entry* pEntry,
	...
	if (read_config_file() == -1) {
		syslog(LOG_ERR,
			   "Warning: Could not read values from config file %s. Using "
			   "defaults.",
			   CONFIG_FILE);
#if defined(LDEBUG)
	...
	if (max_consecutive_per_class != 0) {
		...
		for (i = 0; i < nLen; i++) {
			...
			if (consec_chars > max_consecutive_per_class) {
				mem_len = realloc_error_message(
					&szErrStr, mem_len,
					strlen(CONSEC_FAIL_SZ) + strlen(pEntry->e_name.bv_val));
...
#endif
#if defined(LDEBUG)
				printf("check_password: Found lower character - quality raise "
					   "%d\n",
					   nQuality);
#endif
			}
...
#endif
#if defined(LDEBUG)
				printf("check_password: Found upper character - quality raise "
					   "%d\n",
					   nQuality);
#endif
			}
...
#endif
#if defined(LDEBUG)
				printf("check_password: Found digit character - quality raise "
					   "%d\n",
					   nQuality);
#endif
			}
...
#endif
#if defined(LDEBUG)
				printf("check_password: Found punctuation character - quality "
					   "raise %d\n",
					   nQuality);
#endif
			}
	...
	if (use_cracklib > 0) {
		...
		if (nErr == 0) {
			...
			if (r != NULL) {
				mem_len = realloc_error_message(
					&szErrStr, mem_len,
					strlen(BAD_PASSWORD_SZ) + strlen(pEntry->e_name.bv_val) +
						strlen(r));
